...
- `b` and `c`: backward and forward cross-immunity (constant in each region). Vector of
   length `M`
- `K`: cross-immunity. `K[i][a,b]` is the immunity to `a` generated by `b` in region `i`.
"""
...
function check_population_size(region::SIRRegion)
	mapreduce(x -> isapprox(x, 1, rtol=1e-15), *, population(region), init=true)
end
function check_population_size(S, I, R, K)
	mapreduce(*, 1:length(S), init=true) do a
		isapprox(population(S, I, R, K, a), 1, rtol=1e-15)
	end
end
"""
...
"""
	cross_immunity(N::Int, backward::Float64, forward::Float64)
"""
function cross_immunity(N::Int, backward::Float64, forward::Float64)
	K = diagm(ones(N))
